...
- **Block streaming** (optional, `[ai]` config): `stream_reply = true` 开启后，文本轮走 `chat_completion_stream`，增量按 `stream_block_chars`（默认 800）或空闲 `stream_block_idle_ms`（默认 1000）分块实时投递，静默探测期（112 字符）内绝不转发（防 NO_REPLY 漏出）。流中途失败抛 `StreamAbortedError`（`core/ai/protocol.py`）：零转发 → 回退下一模型（失败记冷却）；已转发部分文本 → 终止并补发剩余尾巴，避免双回复与丢结尾。服务内部降级重试（如网关拒 `stream_options`）前触发 `StreamCallbacks.on_reset`，ToolLoop 归零转发偏移，新生成从 0 开始。
...
- **实现计划与架构文档**：需要了解计划、差距清单或文档状态时，先阅读 `docs/AGENTS.md`。
...
- `docs/` is an agent-only working directory for implementation plans, architecture notes, and temporary research. Keep it ignored and do not commit its contents. Read `docs/AGENTS.md` for document navigation and status conventions.
