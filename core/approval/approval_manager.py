...
class ApprovalManager:
    ...
    def add_to_whitelist(self, tool_name: str, target: str):
        ...
        if tool_name in ("read_file", "write_file", "edit_file", "apply_patch"):
            ...
            if not any(
                e.get("command") == cmd_name for e in self._whitelist["exec_commands"]
            ):
                self._whitelist["exec_commands"].append(
                    {"command": cmd_name, "approved_at": now}
                )
            # v2 allowlist 条目（bare-name pattern）
            self._whitelist.setdefault("allowlist", [])
            if not any(
                e.get("pattern") == cmd_name for e in self._whitelist["allowlist"]
            ):
    ...
    async def request_approval(
        ...
        *,
        ...
        if plan:
            self._pending_plans[session_key] = plan
        ...
        try:
            sent = await sender.send(chat_type="c2c", chat_id=admin_id, req=req)
        except Exception as e:
            _log.warning("审批消息发送异常: %s", e)
            sent = False
        if not sent:
