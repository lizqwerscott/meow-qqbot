...
@command(
    ...
    aliases=[],
    permission="admin",
    description="自定义表情描述和标签。用法：猫猫 /表情编辑 <hash> 描述=xxx 标签=xxx",
)
class EmojiEditCommand:
    ...
    async def execute(
        ...
        if len(parts) < 2:
            return make_reply(
                input_message, "格式：猫猫 /表情编辑 <hash> 描述=xxx 标签=A、B"
            )
        ...
        if record is None:
            return make_reply(
                input_message,
                f"未找到表情「{emoji_hash}」。「猫猫 /表情列表」查看所有。",
            )

        ok = await self.emoji_manager.set_custom(
            record["hash"], description=desc, tags=tags
        )
        if ok:
            changes = []
            if desc is not None:
            ...
            return make_reply(
                input_message,
                f"表情 {record['hash'][:12]}.. 已更新：{'；'.join(changes)}",
            )
        else:
