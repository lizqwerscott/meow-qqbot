...
@command(
    name="表情查看",
    aliases=["emoji"],
    description="查看指定表情的详细信息。用法：猫猫 /表情查看 <hash>",
)
class EmojiInfoCommand:
    ...
    async def execute(
        ...
        if not emoji_hash:
            return make_reply(
                input_message, "请提供表情 hash。用法：猫猫 /表情查看 <hash>"
            )
        ...
        has_custom = (
            record.get("user_description") is not None
            or record.get("user_tags") is not None
        )
        if has_custom:
            lines.append(f"")
