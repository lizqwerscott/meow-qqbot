...
class ChatContextManager:
    ...
    def register_default_command(self, command_manager: CommandManager):
        # 历史命令
        command_manager.register_command(
            Command(
                name="历史",
                handler=self.handle_history_command,
                aliases=["history"],
        ...
        command_manager.register_command(
            Command(
                name="清空",
                handler=self.handle_clear_command,
                aliases=["clear"],
                permission=PermissionLevel.DEFAULT,
        ...
        command_manager.register_command(
            Command(
                name="聊天列表",
                handler=self.handle_list_chats_command,
                aliases=["list", "chats"],
    ...
    def handle_list_chats_command(
        ...
        try:
            ...
            if not active_chats:
                ...
                # 格式化聊天列表
                chats_text = ["活跃聊天列表："]
                for i, (chat_id_item, chat_context) in enumerate(
                    active_chats.items(), 1
                ):
