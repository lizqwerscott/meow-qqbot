...
class JargonMiner:
    ...
    def __init__(
        ...
        self._inference_thresholds: List[int] = cfg.get(
        ...
        for term in set(tokens):
            ...
            if existing:
                await self._update_entry(
                    existing, chat_id, message_text, chat_count, now
                )
            else:
    ...
    async def _infer_meaning(self, entry: JargonEntry) -> None:
        ...
        try:
            ...
            if response_text:
                ...
                _log.info(f"JargonMiner 推理 [{entry.term}]: {entry.definition}")
            else:
    ...
    def get_active_jargons(self, chat_id: str) -> List[JargonEntry]:
        ...
        entries.sort(
            key=lambda e: (e.inference_level, e.source != "manual"), reverse=True
        )
        return entries[:20]
