...
from core.tools.impl.file import is_admin_private, sandbox_target, _approve_path_access
...
def create_search_entries(deps: ToolDeps) -> list[ToolEntry]:
    ...
    async def _run_subprocess(
        ...
        try:
            ...
            if proc is not None and proc.returncode is None:
                proc.kill()
                await proc.wait()
            return ToolResult(content=json.dumps({"error": f"搜索超时（{timeout}秒）"}, ensure_ascii=False))
        except Exception as e:
            if proc is not None and proc.returncode is None:
                proc.kill()
                await proc.wait()
            return ToolResult(content=json.dumps({"error": f"搜索失败: {e}"}, ensure_ascii=False))
    ...
    async def _search_content(args: dict, ctx: ToolContext) -> ToolResult:
        ...
        if not pattern:
            ...
            return ToolResult(content=json.dumps({"error": "请提供 pattern"}, ensure_ascii=False))

        rg = _ensure_rg()
        if not rg:
            _log.warning("rg not found, can't search_content")
            return ToolResult(content=json.dumps({"error": "rg (ripgrep) 未安装，无法搜索文件内容"}, ensure_ascii=False))
        ...
        try:
            target = sandbox_target(ctx.is_group, ctx.chat_id, search_path, deps, admin_override=admin)
        except ValueError as e:
            approved = await _approve_path_access(ctx, search_path, "search_content", str(e), deps)
        ...
        if not target.exists():
            return ToolResult(content=json.dumps({"error": f"路径不存在: {search_path}"}, ensure_ascii=False))
        ...
        _log.info("search_content: %s %s in %s", pattern[:60], " (literal)" if literal else "", search_path)
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if proc.returncode is None:
                        proc.kill()
                        await proc.wait()
                    return ToolResult(content=json.dumps({"error": "搜索超时（30秒）"}, ensure_ascii=False))
                ...
                if event.get("type") == "match":
                    ...
                    if len(matches) < limit and match_count <= SAFETY_LIMIT:
                        ...
                        rel_path = os.path.relpath(abs_path, base_path) if abs_path != "?" else "?"
                    ...
                    if match_count > SAFETY_LIMIT:
                        ...
                        _log.info("search_content: %d+ matches for %s (safety limit)", match_count, pattern[:60])
            ...
            if proc.returncode not in (0, 1) and stderr_text:
                ...
                return ToolResult(content=json.dumps({"error": stderr_text}, ensure_ascii=False))
    ...
    async def _find_files(args: dict, ctx: ToolContext) -> ToolResult:
        ...
        if not raw_pattern:
            ...
            return ToolResult(content=json.dumps({"error": "请提供 pattern（glob 模式）"}, ensure_ascii=False))

        fd = _ensure_fd()
        if not fd:
            _log.warning("fd not found, can't find_files")
            return ToolResult(content=json.dumps({"error": "fd 未安装，无法搜索文件"}, ensure_ascii=False))
        ...
        try:
            ...
            approved = await _approve_path_access(ctx, search_path, "find_files", str(e), deps)
            if approved is not None:
                target = approved
            else:
        ...
        if not target.is_dir():
            return ToolResult(content=json.dumps({"error": f"'{search_path}' 不是目录"}, ensure_ascii=False))
        ...
        if returncode != 0 and stderr_text.strip():
            ...
            return ToolResult(content=json.dumps({"error": stderr_text.strip()}, ensure_ascii=False))
        ...
        result_data = {
            "success": True, "files": relativized[:limit],
            "total": total, "limit_reached": total > limit,
        }
        return ToolResult(content=json.dumps(result_data, ensure_ascii=False))
    ...
    SEARCH_CONTENT_PARAMS = {
        ...
        "properties": {
            ...
            "path": {"type": "string", "description": "工作区内的相对路径，默认当前目录"},
            "glob": {"type": "string", "description": "文件名过滤 glob，如 *.py"},
            "ignore_case": {"type": "boolean", "description": "忽略大小写，默认 false"},
            "literal": {"type": "boolean", "description": "字面匹配（非正则），默认 false"},
            "limit": {"type": "integer", "description": "最大结果数，默认 50，最大 500"},
        },
        "required": ["pattern"],
    }
    ...
    FIND_FILES_PARAMS = {
        ...
        "properties": {
            ...
            "path": {"type": "string", "description": "工作区内的相对路径，默认当前目录"},
            "limit": {"type": "integer", "description": "最大结果数，默认 200，最大 2000"},
        },
        "required": ["pattern"],
    }
